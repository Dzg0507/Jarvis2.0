...
export default function CyberpunkChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      content:
        "J.A.R.V.I.S. online and ready to assist. I have access to a variety of tools to help you with your tasks. How can I help you today?",
      isUser: false,
      timestamp: new Date(),
    },
  ])
  ...
  const handleSendMessage = async (promptOverride?: string) => {
    ...
    try {
      ...
      const SHELL_COMMAND_PREFIX = "EXECUTE_SHELL_COMMAND_PERMISSION_REQUEST:::";
      if (aiContent.startsWith(SHELL_COMMAND_PREFIX)) {
        const command = aiContent.substring(SHELL_COMMAND_PREFIX.length);
        setCommandToExecute(command);
        setShowPermissionPrompt(true);
        setIsThinking(false); // Stop thinking animation for permission prompt
        setActiveTool("");
        return; // Stop further processing of AI response
      }
      ...
      const errorResponse: Message = {
        id: `${Date.now()}_error`,
        content: "I seem to be having trouble processing that request. It's possible that the tool required for this task is unavailable or that there is a network issue. Please try again shortly.",
        isUser: false,
        timestamp: new Date(),
      };
      setMessages((prev) => [...prev, errorResponse]);
